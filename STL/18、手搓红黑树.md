...
class RBTree{
    ...
    public:
        ...
        Value value;
        Color color;
        explicit RBNode(const Key& key,const Value&value)
                :key(key),value(value),
                 color(Color::RED),
        ...
        RBNode():
                color(Color::BLACK),left(nullptr),
...
private:
    ...
    void transPlant(RBNode* u,RBNode* v){//u是删除的，v是替换的
        if(u->parent==nil){
            root = v;
    ...
    void deleteFixUp(RBNode*x){
        ...
        while(x!=root&&(x->color==Color::BLACK||x==nil)){
            if(x->parent->left==x){
                ...
                if(w->left->color==Color::BLACK&&w->right
                                                         ->color==Color::BLACK){
                    w->color=Color::RED;
                    x = x->parent;
                }else{
    ...
    void deleteNode(RBNode* z){
        ...
        if(y->left==nil){
            ...
            if(y->parent!=z){
                transPlant(y,x);//x替换y
                y->right = z->right;//z的右子树接到y
                y->right->parent = y;
            }
            transPlant(z,y);//y替换x
            y->left = z->left;//将z的左子树接到y
            y->left->parent = y;
            y->color = z->color;//保存z颜色
        }
...
public:
    ...
    void remove(const Key& key){
        RBNode* delNode = searchRBNode(key);
        if(delNode !=nil){
            deleteNode(delNode);
        }
    }
...
本文转自 <https://blog.csdn.net/2401_82911768/article/details/147426122?spm=1001.2014.3001.5502>，如有侵权，请联系删除。